...
def func(data,labels):
    dic = {'high':0,'mid':1,'low':2,'excellent':3,'good':4,'fair':5,'poor':6,'stable':7,'mod-stable':8,'unstable':9}
    for i in data:
    ...
    for i in labels:
        if i in dic2:
            labels_res.append(dic2[i])
    return data,labels_res

data,labels = load_data('post-operative.data')
dataArr, classLabels=func(data,labels)
...
def testRbf(k1 = 1.3):
    ...
    Returns:
        无
    """
    data,labels = load_data('post_operative.data')
    dataArr, labelArr=func(data,labels)                       #加载训练集
    ...
    data,labels = load_data('post-operative2.data')
    dataArr, labelArr=func(data,labels)                        #加载测试集
    errorCount = 0
    datMat = np.mat(dataArr); labelMat = np.mat(labelArr).transpose()
    m,n = np.shape(datMat)
    for i in range(m):
